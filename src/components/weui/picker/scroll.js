/*
 * Created by jinhuailee on Thu Oct 19 2017 12:32:45
 *
...
$.fn.scroll = function (options) {
  ...
  const items = defaults.items.map((item) => {
    return `<div class="weui-picker__item${item.disabled ? ' weui-picker__item_disabled' : ''}">${typeof item === 'object' ? '['+item.value+']'+item.label : item}</div>`
  }).join('')
  const $this = $(this)
